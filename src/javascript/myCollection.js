$(document).ready(function(){
    $.post("../php/myCollection1.php", function (data) {
        data = JSON.parse(data);
        if(data[0].length!==0){
            favoriteShow(data,1);
        }
        else favoriteShow(data,0);

    })
    function favoriteShow(data,pageNumber) {
        let resultBox = document.getElementById("myCollectionBox");
        ...
        if(pageNumber!==0) {
            if (pageNumber !== 1) {
                ...
                previous.onclick = function () {
                    favoriteShow(data, pageNumber - 1)
                };
        ...
        else resultBox.innerHTML="<p>你没有收藏的照片！</p>"
    }
    function deleteFavor(imageID) {
        $.post("../php/collectionDelete.php",{imageID:imageID}, function (data) {
