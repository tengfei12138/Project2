$(document).ready(function(){
    $.post("../php/myPhoto1.php", function (data) {
        data = JSON.parse(data);
        if(data[0].length!==0){
            myPhotoShow(data,1);
        }
        else myPhotoShow(data,0);

    })
    function myPhotoShow(data,pageNumber) {
        ...
        else resultBox.innerHTML="<p>你没有上传照片！</p>"
    }
    function deletePhoto(imageID) {
        $.post("../php/myPhoto3.php",{imageID:imageID}, function (data) {
            window.location.reload();
            console.log(data);
        })
    }
    function modifyPhoto(imageID) {
        window.location.href="../html/photoModify.php?imageID="+imageID;
    }
});